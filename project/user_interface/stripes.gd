...
func _draw():
	...
	for note in 128:
		...
		if note % 12 in [1, 3, 6, 8, 10]:
			...
			draw_line(Vector2(0, height), Vector2(end, height), color, subdivision)
		#if its a BC or EF, so we draw a line between them
		if note % 12 in [4, 11]:
			draw_line(Vector2(0, (127 - note) * one_seminote_gap), Vector2(end, (127 - note) * one_seminote_gap), color, 2)
